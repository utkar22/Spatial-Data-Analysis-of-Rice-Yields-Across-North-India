...
df <- read_dta("C:\\Users\\Utkarsh\\Documents\\homework\\SSSE\\Project\\Data\\Final1_TGP.dta")
...
plot(yield.vgm,
     ...
     type="b",
     main = "Variogram for non-detrended yield data (Trans Indo-Gangetic Plain)"
)
...
plot(yield.vgm.detrend.x,
     ...
     ylab="Gamma(h), Semivariance",
     type="b",
     main = "Variogram for yield data detrended on longitude (Trans Indo-Gangetic Plain)"
)
...
plot(yield.vgm.detrend.y,
     ...
     ylab="Gamma(h), Semivariance",
     type="b",
     main = "Variogram for yield data detrended on latitude (Trans Indo-Gangetic Plain)"
)
...
plotly::plot_ly(type="scatter", mode="lines+markers") %>%
  ...
  layout(title = "Variograms for Trans Indo-Gangetic Plain",
