...
plot(yield.vgm,
     ...
     type="b",
     main = "Variogram for non-detrended yield data (Middle Gangetic Plain)"
)
...
plot(yield.vgm.detrend.x,
     ...
     ylab="Gamma(h), Semivariance",
     type="b",
     main = "Variogram for yield data detrended on longitude (Middle Gangetic Plain)"
)
...
plot(yield.vgm.detrend.y,
     ...
     ylab="Gamma(h), Semivariance",
     type="b",
     main = "Variogram for yield data detrended on latitude (Middle Gangetic Plain)"
)
...
plot(yield.vgm.detrend.xy,
     ...
     ylab="Gamma(h), Semivariance",
     type="b",
     main = "Variogram for yield data detrended on both latitude and longitude (Middle Gangetic Plain)"
)
...
plot(yield.vgm.detrend.full,
     ...
     ylab="Gamma(h), Semivariance",
     type="b",
     main = "Variogram for yield data detrended on all explanatory variables (Middle Gangetic Plain)"
)
...
plotly::plot_ly(type="scatter", mode="lines+markers") %>%
  ...
  layout(title = "Variograms for Middle Gangetic Plain",
...
yield.fit.sp = fit.variogram(yield.vgm, model = vgm(5, "Sph", 1, 15))
# View Results
yield.fit.sp
